...
func SetupANDRun() error {
	fs.Fs_open()
	core.CreateUserSchema()
	core.InitSuperAdminUser()
	// Create app
	app := fiber.New()
	// Attach middleware
	app.Use(recover.New())
	...
	app.Get("/metrics", monitor.New(monitor.Config{Title: "User manager service"}))
	...
	if err := app.Listen("localhost:8981"); err != nil {
		log.Panic(err)
	}
